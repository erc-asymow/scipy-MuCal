...
;
        ...
        const double cosalpharec = recplus.x()*recminus.x() + recplus.y()*recminus.y() + recplus.z()*recminus.z();
        const double cosalphagen = genplus.x()*genminus.x() + genplus.y()*genminus.y() + genplus.z()*genminus.z();
        
        return cosalpharec/cosalphagen;
    }
""")
...
d = d.Define("krplus", "(M_PI_2 - 2.0*atan(exp(-Mupluscons_eta))) - (M_PI_2 - 2.0*atan(exp(-Muplusgen_eta)))");
d = d.Define("krminus", "(M_PI_2 - 2.0*atan(exp(-Muminuscons_eta))) - (M_PI_2 - 2.0*atan(exp(-Muminusgen_eta)))");
