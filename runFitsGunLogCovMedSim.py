...
def nllbinnedmodel(parms, scales, covs, ks, qs):
    ...
    #nll += a**2
    #nll += c**2
    nll += 0.5*np.sum(g1**2)/5.**2
    #nll += 0.5*np.sum(g2**2)
    nll += 0.5*np.sum(g2**2)/5.**2
    nll += 0.5*np.sum(g3**2)/5.**2
    #nll += 0.5*d1**2
    #nll += g2**2
    #nll += d2**2
    ##nll += g3**2
...
def dofit(xbinned,covsbinned):
    ...
    for ieta,(etaL, etaH, etaC) in enumerate(zip(etasL, etasH, etasC)):
        ...
        xlocal = np.zeros((5,nelems),dtype=np.float64)
        ...
        x = pmin(fg, x, (scales,fitcovs,ksfit,qs), doParallel=False, jac=True, h=h, edmtol = 1e-4)
        ...
        ksfinescale = ksfine[np.newaxis,:,np.newaxis]
