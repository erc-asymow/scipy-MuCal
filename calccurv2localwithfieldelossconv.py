...
#qop = qop0
qop = q/sympy.sqrt(p0*p0 + dEdx*dEdx*s*s + 2*sympy.sqrt(p0*p0+mass*mass)*dEdx*s)
...
print("shat")
shat = sympy.solve(I1.dot(M-r1), s)[0]
#shat = shat.simplify()
...
for inparm in inparms:
    ...
    dsdinparm = dsdinparm.subs(subsconst)
...
for parm,parmlabel in zip(parms, parmlabels):
    ...
    dparmds = dparmds.subs(subsconst)
    for inparm,inparmlabel,dsdinparm in zip(inparms,inparmlabels,dsdinparms):
        ...
        #for jinparm, jinparmlabel in zip(
