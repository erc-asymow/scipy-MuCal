...
def makeData(inputFile,ptmin):
    ...
    #cut = f"1./gen_curv>{ptmin} && fabs(gen_eta)<2.4 && reco_charge>-2. && tkHits>15"
    ...
    d = d.Define('kr', "covrphi(0,0)/reco_curv/reco_curv")
    #d = d.Define('kr', "tkptErr*tkptErr*reco_curv*reco_curv/gen_curv/gen_curv")
    
    #d = d.Define('kr', "gen_curv/reco_curv")
    d = d.Define('kgen', "gen_curv")
    #d = d.Define('kgen', "reco_curv")
    
    #d = d.Filter("kr>=0. && kr<0.3")
...
def makeDataDiMuon(inputFile, idx, ptmin):
    ...
    d = d.Define('kr', f'covrphi{idx}(0,0)*pt{idx}*pt{idx}')
    #d = d.Define('kr', f'cErr{idx}*cErr{idx}*mcpt{idx}*mcpt{idx}/pt{idx}/pt{idx}')
...
def sigmasq(a,c,g1,d1,g2,d2,g3,d3,k):
    #l = computeTrackLength(eta)
    ...
    #res = a + c/k**2  + g1*k**2 + g2/(1.+d2*k**2) + g3/(1.+d3*k**2)
    #res = a + c/k**2 + g2/(1.+d2*k**2) + g3/(1.+d3*k**2)
    #res = a + c/k**2 + g2/k**2/(1.+d2*k**2) + g3/k**2/(1.+d3*k**2)
    res = a + c/k**2 + g2/(1.+d2*k**2) + g3/(1.+d3*k**2)
...
def nllbinnedmodel(parms, scales, errs, ks):
    parms = parms*parmscale
    ...
    mval = sigmasq(a,c,g1,d1,g2,d2,g3,d3,ks)
...
#etas = np.linspace(-0.3,0, nEtaBins+1)
...
xmu = 1e-3*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
xsigma = 1e-3*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
...
def scalesigma(x, etasc, ks):
    ...
    scaleout = sigmasq(a,c,g1,d1,g2,d2,g3,d3, ks) + 0.*qs
...
for ieta,(etaL, etaH, etaC) in enumerate(zip(etasL, etasH, etasC)):
    ...
    x = pmin(fg, x, (scales,errs,ksfit), doParallel=False, jac=True, h=None,xtol=1e-14,edmtol=1e-3)
    x = pmin(fg, x, (scales,errs,ksfit), doParallel=False, jac=True, h=h, edmtol = 1e-4)
    
    print("computing hess")
    hess = h(x, scales, errs,ksfit)
...
onp.savez_compressed("unbinnedfiterrs.npz",
                     xbinned = xbinned,
