...
subsconst = [(qop0, qop0val), (lam0, lam0val), (phi0, phi0val), (xt0, xt0val), (yt0, yt0val), (Bx, Bxval), (By, Byval), (Bz, Bzval), (s, sval), (xi, 0)]
subsrev = [(qop0val, qop0), (Bxval, Bx), (Byval, By), (Bzval, Bz), (sval, s), (xi,0)]
...
#xifact = 1.+xi
xifact = sympy.exp(xi)
qop = q/sympy.sqrt(p0*p0 + xifact*xifact*dEdx*dEdx*s*s + 2*sympy.sqrt(p0*p0+mass*mass)*xifact*dEdx*s)
...
print("P")
P = (q/qop)*T
P = P.subs(subsconst).subs(subsrev)
P = P.simplify()

print("V")
#X = M.subs(subsconst).subs(subsrev)
#X = M0.subs(subsconst).subs(subsrev)
#X = M0const + gamma*(theta-sintheta)/Q*H + sintheta/Q*T0 + alpha*(1-costheta)/Q*N0
X = gamma*(theta-sintheta)/Q*H + sintheta/Q*T0 + alpha*(1-costheta)/Q*N0
X = X.subs(subsconst).subs(subsrev)
X = X.simplify()
...
cartparms = [Xx, Xy, Xz, Px, Py, Pz]
cartparmlabels = ["x", "y", "z", "px", "py", "pz"]
...
substitutionscart, resultscart = sympy.cse(cartparms, symbols = numbered_symbols("xc"))
for sub in substitutionscart:
  #print(sub[1])
  cxxsub = cxxcode(sub[1],standard='C++17')
  print(f"const double {sub[0]} = {cxxsub};")
for res,label in zip(resultscart,cartparmlabels):
  cxxres = cxxcode(res,standard='C++17')
  print(f"const double {label} = {cxxres};")


print(f"Eigen::Matrix<double, {len(cartparms)}, 1> rescart;")
for i,parmlabel in enumerate(cartparmlabels):
    print(f"rescart[{i}] = {parmlabel};")
