...
for track in tree:
  ...
  for i in range(nhits):
    ...
    #assert(0)
      ...
      if yerrs[i]<1e3:
        Vinvfull[5*i+4,5*i+4] = yerrsinv[i]
    #Vinvfull[5*i+3:5*i+5,5*i+3:5*i+5] = Vtmp
    #dyfull[5*i+3,0] = dx[i]
    #dyfull[5*i+4,0] = dy[i]
    
    dyfull[5*i+3,0] = x[i] - ustate[i,3]
    dyfull[5*i+4,0] = y[i] - ustate[i,4]
    ...
    if i > 0:
      ...
      #Ffull[5*i:5*(i+1),5*(i-1):5*i] = np.linalg.inv(F[i])
      #Qinvfull[5*i:5*i+3,5*i:5*i+3] = Qinv[i]
      Qinvfull[5*i:5*i+3,5*i:5*i+3] = np.linalg.inv(Q[i])
  ...
  #dchi2dxhit = -2.*Hfull.transpose() @ Vinvfull @ Hfull @ dyfull
  ...
  #dchi2dxmom = 2.*(Ifull-Ffull).transpose()@Hfull.transpose()@Qinvfull@Hfull@(Ifull-Ffull)@x0full
  ...
  #print(d2chi2dx2.shape)
  #print(dchi2dx.shape)
  #xout = np.linalg.solve(d2chi2dx2, -dchi2dx)
  ...
  #dchi2dlambda = P @ Hfull @ (Ifull-Ffull) @ x0full
