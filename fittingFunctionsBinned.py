...
def defineState(nEtaBins,nPtBins):
    ...
    sigma = np.full((nEtaBins,nEtaBins,nPtBins,nPtBins),-3.5) 
    nsig = np.full((nEtaBins,nEtaBins,nPtBins,nPtBins),14.)
...
def kernelpdf(scale, sigma, nsig, dataset, datasetGen):
    ...
    valsGen = np.linspace(2.9,3.3,1000)
    ...
    h_ext = np.swapaxes(np.swapaxes(h,2,4),3,4)[:,:,np.newaxis,:,np.newaxis,np.newaxis,:,:]
    
    #print h_ext.shape, "h_ext.shape"

    sigma_ext = sigma[:,:,np.newaxis,np.newaxis,np.newaxis,np.newaxis,:,:]

    xscale = np.sqrt(2.)*sigma_ext

    #print xscale.shape, "xscale.shape"

    maxR_ext = maxR[np.newaxis,np.newaxis,:,np.newaxis,np.newaxis, np.newaxis,np.newaxis,np.newaxis]
    minR_ext = minR[np.newaxis,np.newaxis,:,np.newaxis,np.newaxis, np.newaxis,np.newaxis,np.newaxis]
    ...
    vals_ext = valsReco[np.newaxis,np.newaxis,:,np.newaxis,np.newaxis, np.newaxis,np.newaxis,np.newaxis]
...
def nll(x,nEtaBins,nPtBins,dataset,datasetGen):
    ...
    #nsig = np.sum(datasetGen,axis=2)[:,:,np.newaxis,:,:,:,:]

    norm_pdf = kernelpdf(scale, sigma, nsig, dataset, datasetGen)
...
def plots(x,nEtaBins,nPtBins,dataset,datasetGen):
    ...
    #sigma = np.exp(np.full((nEtaBins,nEtaBins,nPtBins,nPtBins),-3.5))
    nsig = np.exp(x[2*sep:].reshape((nEtaBins,nEtaBins,nPtBins,nPtBins)))
    #nsig = np.sum(dataset,axis=2)[:,:,np.newaxis,:,:,:,:]
    
    #print dataset.shape, 'eta1,eta2,mass,phi2,phi1,pt2,pt1'

    pdf = kernelpdf(scale, sigma, nsig, dataset, datasetGen)
        ...
        for ieta2 in range(nEtaBins):
            for ipt1 in range(nPtBins):
                for ipt2 in range(nPtBins):
                    ...
                    nsig_true = np.sum(dataset,axis=2)[ieta1,ieta2,0,0,ipt1,ipt2]


                    plt.clf()
                  
                    fig, (ax1, ax2) = plt.subplots(nrows=2)
                    ax1.errorbar(mass, dataset[ieta1,ieta2,:,0,0,ipt1,ipt2], yerr=np.sqrt(dataset[ieta1,ieta2,:,0,0,ipt1,ipt2]), fmt='.')
                    ...
                    ax1.plot(mass, pdf[ieta1,ieta2,:,0,0,ipt1,ipt2])
                    plt.xlim(minR, maxR)
        
                    ax2.errorbar(mass,dataset[ieta1,ieta2,:,0,0,ipt1,ipt2]/pdf[ieta1,ieta2,:,0,0,ipt1,ipt2],yerr=np.sqrt(dataset[ieta1,ieta2,:,0,0,ipt1,ipt2])/pdf[ieta1,ieta2,:,0,0,ipt1,ipt2], fmt='.')
