...
def makeDataDiMuon(inputFile, idx, ptmin):
    ...
    #d = d.Define('kr', f'dxy{idx}_bmsp')
    #d = d.Define('kr', f'cov{idx}_03/sqrt(cov{idx}_00*cov{idx}_33)')
...
def nll(parms,dataset,eta,ntotal):
    ...
    scalem = sigmasq(a,b,c,d,kgen,eta) + 0*q
    resm = s*scalem
...
def scalesigma(x, etasc, ks):
    ...
    a = x[...,0,np.newaxis,np.newaxis]
    b = x[...,1,np.newaxis,np.newaxis]
    c = x[...,2,np.newaxis,np.newaxis]
    d = x[...,3,np.newaxis,np.newaxis]
    s = x[...,4,np.newaxis,np.newaxis]
    ...
    scaleout = sigmasq(a,b,c,d, ks, etasc[...,np.newaxis,np.newaxis]) + 0.*qs
    #sigmaout = np.sqrt(sigmasqout)
    sigmaout = s*scaleout/scaleout
