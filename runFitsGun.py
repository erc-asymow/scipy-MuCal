...
def makeData(inputFile,ptmin):
    ...
    d = d.Define('kr', "reco_curv/gen_curv")
...
def scale(A,e,M,W,Y,Z,V,e2,W2,Z2,k,q,eta):
...
def nll(parms,dataset,eta):
    ...
    scalem = scale(A,e,M,W,Y,Z,V,e2,W2,Z2,kgen, q, eta)
...
def scalesigma(x, etasc, ks):
    ...
    a = x[...,6,np.newaxis,np.newaxis]
    b = x[...,7,np.newaxis,np.newaxis]
    c = x[...,8,np.newaxis,np.newaxis]
    d = x[...,9,np.newaxis,np.newaxis]
    ...
    scaleout = scale(A,e,M,W,Y,Z,V,e2,W2,Z2,ks, qs, etasc[...,np.newaxis,np.newaxis])
...
for ieta,(etaL, etaH, etaC) in enumerate(zip(etasL, etasH, etasC)):
    ...
    x = pmin(fg, x, (dsetbin,etaC), doParallel=False, jac=True, h=None)
    x = pmin(fg, x, (dsetbin,etaC), doParallel=False, jac=True, h=h)
