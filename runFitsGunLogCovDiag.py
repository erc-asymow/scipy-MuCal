...
#double eigval(const std::tuple<SVector5, SMatrix55> &eig) {
    ...
    #//return e(imax);
    #//return log(e(imax));
    #//return 1./std::abs(v(0,imax));
    #return v(0,imax)*v(0,imax);
...
#double rphicov(double c00, double c01, double c02, double c03, double c04, double c11, double c12, double c13, double c14, double c22, double c23, double c24, double c33, double c34, double c44, double k, double eta) {
...
def makeData(inputFile,ptmin):
    ...
    #d = d.Define("kr", "rphicov(cov00,cov01,cov02,cov03,cov04,cov11,cov12,cov13,cov14,cov22,cov23,cov24,cov33,cov34,cov44,reco_curv,reco_eta)")

    #d = d.Define("kr", "covrphi(2,2)")
    d = d.Define("kr", "eigdiag(covrphi,gen_eta)")
    #d = d.Define("kr", "std::get<0>(eig)[1]")
    #d = d.Define("kr", "cov00 + cov22 - 2.*cov02")
    #d = d.Define("kr", "(1.-0.13*0.13)*cov22 + 0.13*0.13*cov00 - 2.*sqrt(1.-0.13*0.13)*0.13*cov02")
    #d = d.Define("kr", "(1.-0.118*0.118)*cov00 + 0.118*0.118*cov22 + 2.*sqrt(1.-0.118*0.118)*0.118*cov02")
    #d = d.Define("kr", "cov00-2.342*cov02")
    #d = d.Define("kr", "-cov03")
    #d = d.Define("kr", "eigval(eig)")
    #d = d.Define("kr", "eigval(eig) - 4.*log(reco_curv)")
...
def makeDataDiMuon(inputFile, idx, ptmin):
    ...
    #d = d.Define("kr", f"rphicov(cov{idx}_00,cov{idx}_01,cov{idx}_02,cov{idx}_03,cov{idx}_04,cov{idx}_11,cov{idx}_12,cov{idx}_13,cov{idx}_14,cov{idx}_22,cov{idx}_23,cov{idx}_24,cov{idx}_33,cov{idx}_34,cov{idx}_44,1./pt{idx},eta{idx})")
    ...
    #d = d.Define('kr', f'covrphi{idx}(2,2)')
    d = d.Define('kr', f'eigdiag(covrphi{idx},mceta{idx})')
    #d = d.Define('kr', f'eigval(eig{idx})')
    #d = d.Define('kr', f"-cov{idx}_03")
    #d = d.Define('kr', f'eigval(eig{idx}) + 4.*log(pt{idx})')
    #d = d.Define('kr', f'cov{idx}_23/sqrt(cov{idx}_22*cov{idx}_33)')
    #d = d.Define('kr', f'cov{idx}_02/sqrt(cov{idx}_00*cov{idx}_22)')
    #d = d.Define("kr", f"(1-1./3./3.)*cov{idx}_22 + (1./3./3.)*cov{idx}_00 + 2.*sqrt(1-1./3./3.)*(1./3.)*cov{idx}_02")
    #d = d.Define("kr", f"(1.-0.13*0.13)*cov{idx}_22 + 0.13*0.13*cov{idx}_00 - 2.*sqrt(1.-0.13*0.13)*0.13*cov{idx}_02")
    #d = d.Define("kr", f"(1.-0.118*0.118)*cov{idx}_00 + 0.118*0.118*cov{idx}_22 + 2.*sqrt(1.-0.118*0.118)*0.118*cov{idx}_02")
...
limcovs = onp.zeros((nEtaBins,3,3),dtype=np.float64)

for i in range(3):
    for j in range(i,3):
        fname = f"unbinnedfiterrsTlog_{i}{j}.npz"
        with np.load(fname) as f:
            xspre = f["xs"]
        vals = xspre[:nEtaBins,0]
        #vals = xspre[:nEtaBins,1]
        #vals = xspre[:nEtaBins,1] + xspre[:nEtaBins,2] + xspre[:nEtaBins,4] + xspre[:nEtaBins,6]
        print("vals shape", vals.shape)
        limcovs[:,i,j] = vals
        limcovs[:,j,i] = vals

e,v = np.linalg.eigh(limcovs)
...
for ieta in range(nEtaBins):
    #initstring += f"SMatrix33 &eigvect = eigvects[{ieta}];\n"
    for i in range(3):
        for j in range(3):
            initstring += f"eigv[{ieta}]({i},{j}) = {v[ieta,i,j]};\n"
            #initstring += f"double &val{i}{j} = eigvect({i},{j});\n"
            #initstring += f"val{i}{j} = {v[ieta,i,j]};\n"
            #initstring += f"eigvect({i},{j}) = {v[ieta,i,j]};\n"
...
std::array<SMatrix33, {nEtaBins}> makeeigvects() {{
    std::array<SMatrix33, {nEtaBins}> eigv;
{initstring}
return eigv;
}}

std::array<SMatrix33, {nEtaBins}> eigvects = makeeigvects();


double eigdiag(const SMatrix33 &cov, double eta) {{
    unsigned int ieta = std::floor((eta + 2.4)/0.1);
    
                                                          
    SMatrix33 outcov = ROOT::Math::Transpose(eigvects[ieta])*cov*eigvects[ieta];
    //SMatrix33 outcov = eigvects[ieta]*cov*ROOT::Math::Transpose(eigvects[ieta]);
    return log(abs(outcov(0,1)));
...
xmu = -12.5*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
xsigma = 10.*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
...
#errsbinned = errsbinned/xbinned**2
#xbinned = 1./xbinned

errsbinned = 1e6*np.exp(xbinned)*errsbinned
xbinned = 1e6*np.exp(xbinned)
...
onp.savez_compressed("unbinnedfiterrseigdiaga01.npz",
                     xbinned = xbinned,
