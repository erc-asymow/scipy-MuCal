...
#A,e,M,a,g,c,d,W,Y,Z = xs.T
#Aerr,eerr,Merr,aerr,gerr,cerr,derr,Werr,Yerr,Zerr = xerrs.T

##plt.figure()
##plt.errorbar(etasc,-2.*W,xerr=0.5*etasw, yerr=2.*Werr, fmt="none")

#plt.figure()
#plt.errorbar(etasc,W,xerr=0.5*etasw, yerr=Werr, fmt="none")
#plt.errorbar(etasc,2.*c**2,xerr=0.5*etasw, yerr=2.*c*cerr, fmt="none")
...
for iparm in range(xs.shape[1]):
    ...
    plt.xlabel("$\eta$")
    plot.savefig(f"plots/parm_{iparm}.pdf")
...
for ieta in range(etasc.shape[0]):
    ...
    plt.savefig(f"plots/scale_{ieta}.png")
    ...
    plt.ylabel("$\delta k/k$")
    plt.title(f"Scale, ${etasl[ieta]:.1f}\leq \eta < {etash[ieta]:.1f}$")
    plt.savefig(f"plots/scalept_{ieta}.png")    
    ...
    if True:
        ...
        plt.ylabel("$\sigma k/k$")
        plt.title(f"Resolution, ${etasl[ieta]:.1f}\leq \eta < {etash[ieta]:.1f}$")
        plt.savefig(f"plots/respt_{ieta}.png")
