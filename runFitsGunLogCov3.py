...
def makeDataDiMuon(inputFile, idx, ptmin):
    ...
    d = d.Define('kr', f'-covrphi{idx}(1,2)')
...
xmu = 0.*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
xsigma = 1e-4*np.ones((nEtaBins,2,nkbins),dtype=np.float64)
...
#errsbinned = 1e32*np.exp(xbinned)*errsbinned
#xbinned = 1e32*np.exp(xbinned)
...
onp.savez_compressed("unbinnedfiterrseigfixed_12.npz",
                     xbinned = xbinned,
