...
def makeData(inputFile,ptmin,ptmax):
    ...
    cut = f"1./reco_curv>{ptmin} && 1./reco_curv<{ptmax} && fabs(gen_eta)<2.4 && reco_charge>-2."
    ...
    d = d.Define('kr', "gen_charge*gen_curv/reco_curv/reco_charge")
    #d = d.Define('kr', "gen_curv/reco_curv")
    d = d.Define('krec', "reco_curv")
...
def makeDataDiMuon(inputFile, idx, ptmin, ptmax):
    ...
    cut = f"pt{idx} > {ptmin} && pt{idx}<{ptmax} && fabs(mceta{idx})<2.4"
...
def scale(A,e,M,W,Y,Z,V,e2,Z2,a,b,c,d,k,q,eta):
    ...
    #res = 1. + A + W/(1.+Y*k**2) + Z2/k**2 + q*M/k + q*Z*k/(1.+Y*k**2) + q*V*k
    #res = 1. + A + W/(1.+Y*k**2) + q*M/k + q*Z/k/(1.+Y*k**2)
    res = 1. + A + W/(1.+Y*k**2) + q*M/k + q*Z*k/(1.+Y*k**2)
...
def sigmasq(a,b,c,d,V,Z2,k,eta):
    l = computeTrackLength(eta)
...
def nll(parms,dataset,eta,ntotal):
    ...
    resmsq = sigmasq(a,b,c,d,V,Z2,kgen,eta)
    resm = np.sqrt(resmsq)
...
dset = onp.stack( (d["eta"],d["q"], d["krec"],d["kr"]), axis=-1)
d = None
dsets.append(dset)
dset = None
...
def scalesigma(x, etasc, ks):
    ...
    sigmasqout = sigmasq(a,b,c,d,V,Z2, ks, etasc[...,np.newaxis,np.newaxis])
