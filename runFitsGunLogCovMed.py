...
def makeDataDiMuon(inputFile, idx, ptmin):
    ...
    #d = d.Define('kr', f'log(-covrphi{idx}(1,2))')
    d = d.Define('kr', f'covrphi{idx}(0,0)')
...
#sort values
sortidxs = onp.lexsort((dataset[:,3],fullidxs),axis=0)
values = dataset[sortidxs,3]
...
end_idxs = np.cumsum(counts)

#for a sorted array, the median is just the middle element
#randomizing the index with a variance of nelements/4 gives the bootstrap uncertainty
nbootstraps = 1000
nmedian = end_idxs - counts//2
nmedboot = end_idxs - counts/2 + np.sqrt(counts/4)*onp.random.standard_normal(size=(nbootstraps,counts.shape[0]))
nmedboot = onp.round(nmedboot).astype(np.int32)

medians = values[nmedian]
medianboots = values[nmedboot]
medianerrs = onp.std(medianboots,axis=0)
medianboots = None

#reshape back to multidimensional binning
medians = onp.reshape(medians, (nEtaBins,2,nkbins))
medianerrs = onp.reshape(medianerrs, (nEtaBins,2,nkbins))
